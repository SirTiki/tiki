...
define('tiki.define', function() {
	var undefined, ret, defineVersion = define.version;
	...
	(function() {
		...
		function require(name, cb, module) {
			...
			if (!cache[key]) {
				var def = defined[name] && defined[name][version];
				if (!def) {
					queue[key] = queue[key] || [];
		...
		var cache = {}, defined = {'tiki.define': {}}, queue = [];
		...
		cache['tiki.define@'+defineVersion] = ret;
	})();
